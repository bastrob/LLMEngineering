    ...
    if finetuning_type == "sft":
        ...
        def format_samples_sft(examples):
            ...
            for instruction, output in zip(examples["instruction"], examples["output"], strict=False):
                message = alpaca_template.format(instruction, output)
                text.append(message)
...
def check_if_huggingface_model_exists(model_id: str, default_value: str = "bastr/TwinLlama-3.1-8B") -> str:
    api = HfApi()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_huggingface_workspace", type=str, default="bastr")
    parser.add_argument("--model_output_huggingface_workspace", type=str, default="bastr")
